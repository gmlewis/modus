...
func memoryBlockAtOffset(wa wasmMemoryReader, offset, sizeOverride uint32) (data []byte, classID byte, words uint32, err error) {
	...
	// All memory blocks from the MoonBit compiler use old-style format
	// Old-style: classID in upper 8 bits, words in lower 24 bits
	...
	// For old-style memory blocks, size = 8 (header) + data
	// Strings use 2 bytes per word (UTF-16), other types use 4 bytes per word
